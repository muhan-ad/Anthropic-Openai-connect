...
编辑 `C:\Users\13238\.claude\settings.json`：

```json
{
  "env": {
    "ANTHROPIC_AUTH_TOKEN": "local-any-token",
...
说明：
- `ANTHROPIC_AUTH_TOKEN` 填任意值即可（本服务默认不校验；若在 `.env` 设置了 `LOCAL_AUTH_TOKEN`，此处必须一致）
...
| `LOCAL_AUTH_TOKEN` | 空 | 本地鉴权 Token，留空不校验 |
...
# 非流式调用（模拟 Claude Code）
$body = @{
  model = "deepseek-v4-flash-0731"
  max_tokens = 64
  messages = @(@{ role = "user"; content = "你好，用一句话自我介绍" })
} | ConvertTo-Json -Depth 5
Invoke-RestMethod -Uri http://127.0.0.1:18880/v1/messages -Method Post -ContentType "application/json" -Body $body | ConvertTo-Json -Depth 6
```
